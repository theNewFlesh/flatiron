...
# install OpenEXR
ENV LD_LIBRARY_PATH="$LD_LIBRARY_PATH:/usr/include/python3.12m/dist-packages"
...
RUN echo "\n${CYAN}INSTALL DEV ENVIRONMENT${CLEAR}"; \
    ...
    x_env_init dev 3.12 && \
    cd /home/ubuntu && \
    ln -s `_x_env_get_path dev 3.12` .dev-env && \
    ln -s `_x_env_get_path dev 3.12`/lib/python3.12/site-packages .dev-packages
